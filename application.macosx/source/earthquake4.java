...
public class earthquake4 extends PApplet {
...
public void setup() {
  ...
  float maxMag= max(PApplet.parseFloat(table.getStringColumn(4)));
  float minMag= min(PApplet.parseFloat(table.getStringColumn(4)));
...
class Earthquake {
  ...
  Earthquake(float latitude, float longitude, String time, float mag, String place, float sz) { //constructor
    pos = new PVector(map(longitude, -180.0f, 180.0f, -width/2 + margin, width/2 - margin), -(map(latitude, -90.0f, 90.0f, -height/2 + margin, height/2 - margin)));
    ...
    day = PApplet.parseInt(time.substring(8,10));
     //println(day);
    hour = PApplet.parseInt(time.substring(11, 13));
    //println(hour);
    minutes = PApplet.parseInt(time.substring(14, 16));
    //println(minutes);
    seconds = PApplet.parseInt(time.substring(17, 19));
    //println(seconds);
...
public void scaleAroundPoint(Earthquake i, float statx, float staty, float scl){
...
//zoom code based off:  http: forum.processing.org/one/topic/zoom-in-out-is-not-smooth.html
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "earthquake4" };
    if (passedArgs != null) {
